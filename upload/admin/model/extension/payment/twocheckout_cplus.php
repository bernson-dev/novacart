...
class ModelExtensionPaymentTwocheckoutCplus extends Model {
	...
	public function install() {
		$this->db->query("
            CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "twocheckout_cplus` (
	...
	public function uninstall() {
		$this->db->query("DROP TABLE IF EXISTS `" . DB_PREFIX . "twocheckout_cplus`;");
	}
	...
	public function refund($order_id, $comment = '') {
		...
		if (strtolower($order['currency_code']) != strtolower($tco_order['Currency'])) {
			return [
				'success' => false,
				'message' => 'Order currency not matching the 2checkout response!'
			];
		}
		if ($order['order_status_id'] == $this->config->get('payment_twocheckout_cplus_refunded_status_id')) {
			return [
				'success' => false,
	...
	private function updateRefundTransaction($order_id, $comment) {
		$this->db->query("UPDATE `" . DB_PREFIX . "twocheckout_cplus` SET 
                `tco_order_status` = 'REFUND',
                `comment` = '" . trim($this->db->escape($comment)) . "'
                WHERE `order_id` = '" . $order_id . "'");

		$this->db->query("UPDATE `" . DB_PREFIX . "order` SET 
                `order_status_id` =  '" . $this->config->get('payment_twocheckout_cplus_refunded_status_id') . "',
                date_modified = NOW() 
                WHERE `order_id` = '" . $order_id . "'");
