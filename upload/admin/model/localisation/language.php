...
class ModelLocalisationLanguage extends Model {
	...
	public function getLanguages($data = array()) {
		// Определяем текущий маршрут, если он задан
		$route = '';
		...
		if ($data) {
			...
			// Добавляем условие выбора только активных языков, кроме настройки языков
			if ($route != 'localisation/language') {
				$sql .= " WHERE status = '1'";
			}
			...
			// Используем отдельный ключ кэша для активных языков
			//$cache_key = 'admin.language';
			//if ($route != 'localisation/language') {
				//$cache_key .= '.active';
			//}
			...
			//if (!$language_data) {
				...
				foreach ($query->rows as $result) {
					// Если не находимся в настройках языков, пропускаем неактивные языки
					if ($route != 'localisation/language' && empty($result['status'])) {
						continue;
					}
