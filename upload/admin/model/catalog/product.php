...
class ModelCatalogProduct extends Model {
	public function addProduct($data) {
		...
		if (!empty($data['main_category_id']) && (int)$data['main_category_id'] > 0) {
			$main_category_id = (int)$data['main_category_id'];

			if (!in_array($main_category_id, $categories, true)) {
				$categories[] = $main_category_id;
			}
		} elseif (!empty($categories)) {
			$main_category_id = (int)$categories[0];
		}
		...
		category_id = '" . (int)$category_id . "',
		main_category = '" . (int)($category_id === $main_category_id) . "'");
		}
	...
	public function getProducts($data = array()) {
		...
		if (isset($data['filter_category']) && !is_null($data['filter_category'])) {
			if (!empty($data['filter_category']) && !empty($data['filter_sub_category'])) {
				...
				$sql .= " AND (" . implode(' OR ', array_unique($implode_data)) . ")";
			} else {
	...
	public function fillEmptyFields($product_id) {
		$product_id = (int)$product_id;
		...
		foreach ($languages as $lang) {
			$languages_map[(int)$lang['language_id']] = $lang;
		}
		...
		$filled_data = array();
		$filled_languages_stats = array();

		foreach ($languages as $lang) {
			$lid = (int)$lang['language_id'];
			$data = $this->getProductDescription($product_id, $lid, $fields);
			$filled_count = $this->countFilledFields($data, $fields);

			$filled_data[$lid] = $data;
			$filled_languages_stats[$lang['code']] = $filled_count;
		...
		$source_language_id = $this->findMostFilledLanguage($filled_data, $fields);

		if (!$source_language_id || empty($filled_data[$source_language_id]) || $this->countFilledFields($filled_data[$source_language_id], $fields) === 0) {
			return array(
		...
		$source = $filled_data[$source_language_id];
		$copied_fields_info = array();

		foreach ($languages as $lang) {
			$target_id = (int)$lang['language_id'];

			if ($target_id === $source_language_id) {
				continue;
			}

			$target = $this->getProductDescription($product_id, $target_id, $fields);
			$updates = array();

			foreach ($fields as $field) {
				$is_target_empty = $this->isFieldEmpty(isset($target[$field]) ? $target[$field] : '');
				$is_source_filled = !$this->isFieldEmpty(isset($source[$field]) ? $source[$field] : '');

				if ($is_target_empty && $is_source_filled) {
					$updates[] = "`" . $field . "` = '" . $this->db->escape($source[$field]) . "'";
					$copied_fields_info[$field][] = $lang['code'];
				}
			}
			...
			if (!empty($updates)) {
				...
				SET " . implode(', ', $updates) . "
				WHERE product_id = '" . (int)$product_id . "'
				AND language_id = '" . (int)$target_id . "'");
			}
		}

		$formatted_copied_fields = array();

		foreach ($copied_fields_info as $field => $languages_copied_to) {
			$formatted_copied_fields[] = array(
			'field' => $field,
			'languages' => $languages_copied_to
			);
		}

		return array(
		'source_language' => isset($languages_map[$source_language_id]) ? array(
		'name' => $languages_map[$source_language_id]['name'],
		'code' => $languages_map[$source_language_id]['code']
		) : null,
		'copied_fields' => $formatted_copied_fields,
	...
	private function productExists($product_id) {
		return $this->db->query("SELECT product_id FROM " . DB_PREFIX . "product WHERE product_id = '{$product_id}'")->num_rows > 0;
	}

	private function getLanguages() {
		return $this->db->query("SELECT language_id, code, name FROM " . DB_PREFIX . "language ORDER BY sort_order ASC, name ASC")->rows;
	}

	private function getProductDescription($product_id, $language_id, $fields) {
		$field_list = implode(',', array_map(function($f) {
			return "`{$f}`";
		}, $fields));
	...
	private function countFilledFields($data, $fields) {
		$count = 0;
		foreach ($fields as $field) {
			if (isset($data[$field]) && !$this->isFieldEmpty($data[$field])) {
				$count++;
			}
		}
		return $count;
	...
	private function findMostFilledLanguage($filled_data, $fields) {
		$max_filled = -1;
		$source_language_id = null;

		foreach ($filled_data as $lid => $data) {
			$count = $this->countFilledFields($data, $fields);
			if ($count > $max_filled) {
				$max_filled = $count;
