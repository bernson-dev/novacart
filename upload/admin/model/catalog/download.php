...
class ModelCatalogDownload extends Model {
	...
	public function getTotalDownloads($data = array()) {
		...
		$sql = "SELECT COUNT(d.download_id) AS total FROM " . DB_PREFIX . "download d LEFT JOIN " . DB_PREFIX . "download_description dd ON (d.download_id = dd.download_id) WHERE dd.language_id = '" . (int)$this->config->get('config_language_id') . "'";
	...
	public function getDownloadReports($download_id, $start = 0, $limit = 10) {
		if ($start < 0) {
			$start = 0;
		}
		...
		$query = $this->db->query("SELECT ip, store_id, date_added FROM " . DB_PREFIX . "download_report WHERE download_id = '" . (int)$download_id . "' ORDER BY date_added ASC LIMIT " . (int)$start . "," . (int)$limit);
		return $query->rows;
	}

	public function deleteDownloadReport(int $download_id) {
		$this->db->query("DELETE FROM `" . DB_PREFIX . "download_report` WHERE `download_id` = '" . (int)$download_id . "'");
	}
