...
class ModelSaleOrder extends Model {
	public function getOrderPhoneMail($order_id) {
	...
	public function getOrdersCountByPhoneEmail($phone, $email) {
		$conditions = array();
		if (!empty($phone)) {
			$phone_clean = preg_replace('/[^0-9]/', '', $phone);
			if (!empty($phone_clean)) {
				$conditions[] = "REPLACE(REPLACE(REPLACE(REPLACE(telephone, '-', ''), '(', ''), ')', ''), ' ', '') LIKE '%" . $this->db->escape($phone_clean) . "%'";
			}
		}
		if (!empty($email)) {
		...
		$sql = "SELECT COUNT(*) as total FROM `" . DB_PREFIX . "order` WHERE " . implode(' OR ', $conditions);
		$query = $this->db->query($sql);
		return $query->row['total'];
	}

	public function getOrdersCountByPhone($phone) {
		if (empty($phone)) {
			return 0;
		}
		$phone_clean = preg_replace('/[^0-9]/', '', $phone);
		...
		WHERE REPLACE(REPLACE(REPLACE(REPLACE(telephone, '-', ''), '(', ''), ')', ''), ' ', '') LIKE '%" . $this->db->escape($phone_clean) . "%'";
		$query = $this->db->query($sql);
		return $query->row['total'];
	}
	...
	public function getOrdersCountByEmail($email) {
		...
		$sql = "SELECT COUNT(*) as total FROM `" . DB_PREFIX . "order` WHERE email = '" . $this->db->escape($email) . "'";
		$query = $this->db->query($sql);
		return $query->row['total'];
	}
	...
	public function getOrders($data = array()) {
		...
		if (!empty($data['filter_order_phone'])) {
			$sql .= " AND REPLACE( REPLACE( REPLACE( REPLACE( o.telephone, '-', '' ), '(', '' ), ')', '' ), ' ', '' ) LIKE '%" . $this->db->escape($data['filter_order_phone']) . "%'";
		}

		if (!empty($data['filter_order_email'])) {
			$sql .= " AND o.email LIKE '%" . $this->db->escape($data['filter_order_email']) . "%'";
		}
	...
	public function getTotalOrders($data = array()) {
		...
		if (!empty($data['filter_order_phone'])) {
			$sql .= " AND REPLACE( REPLACE( REPLACE( REPLACE( telephone, '-', '' ), '(', '' ), ')', '' ), ' ', '' ) LIKE '%" . $this->db->escape($data['filter_order_phone']) . "%'";
		}

		if (!empty($data['filter_order_email'])) {
			$sql .= " AND email LIKE '%" . $this->db->escape($data['filter_order_email']) . "%'";
		}
	...
	public function getOrderCustomers($filter_name) {
		$filter_name = $this->db->escape($filter_name);

		$sql = "SELECT DISTINCT CONCAT(firstname, ' ', lastname) AS customer
            FROM `" . DB_PREFIX . "order`
            WHERE CONCAT(firstname, ' ', lastname) LIKE '%" . $filter_name . "%'
            ORDER BY customer ASC
            LIMIT 0," . (int)$this->config->get('config_limit_autocomplete');
	...
	public function getOrderTelephones($filter_name) {
		$filter_name = $this->db->escape($filter_name);

		$norm = "REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(telephone, '+', ''), '-', ''), '(', ''), ')', ''), ' ', '')";

		$sql = "SELECT telephone AS raw, " . $norm . " AS telephone
			FROM `" . DB_PREFIX . "order`
			WHERE " . $norm . " LIKE '%" . $filter_name . "%'
				AND telephone <> ''
			GROUP BY " . $norm . "
			ORDER BY raw ASC
			LIMIT 0," . (int)$this->config->get('config_limit_autocomplete');
