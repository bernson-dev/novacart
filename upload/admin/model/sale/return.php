...
class ModelSaleReturn extends Model {
	...
	public function getReturns($data = array()) {
		...
		// Выбираем название подписки, если запрошен фильтр по регулярным платежам
		if (!empty($data['filter_recurring'])) {
			$sql .= ", rd.name AS recurring";
		}
		...
		// JOIN таблиц регулярных платежей подключается только при необходимости
		if (!empty($data['filter_recurring'])) {
			$sql .= " LEFT JOIN `" . DB_PREFIX . "order_recurring` `or`
                      ON (r.order_id = `or`.`order_id` AND r.product_id = `or`.`product_id`)
                  LEFT JOIN `" . DB_PREFIX . "recurring` rr
                      ON (`or`.`recurring_id` = rr.`recurring_id`)
                  LEFT JOIN `" . DB_PREFIX . "recurring_description` rd
                      ON (rr.`recurring_id` = rd.`recurring_id`
		...
		if (!empty($data['filter_recurring'])) {
			$implode[] = "rd.name LIKE '" . $this->db->escape($data['filter_recurring']) . "%'";
		}
		...
		// Группировка исключает дублирование возвратов
