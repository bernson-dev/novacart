...
class ControllerExtensionExtensionModule extends Controller {
	...
	public function install() {
		...
		if ($this->validate()) {
			...
			// Сохраняем код установленного модуля в сессии, чтобы потом передать в шаблон
	...
	protected function getList() {
		...
		if (isset($this->session->data['installed_module_code'])) {
			$data['installed_module_code'] = $this->session->data['installed_module_code'];
			unset($this->session->data['installed_module_code']);
		} else {
			$data['installed_module_code'] = '';
		}
		//---
		if ($files) {
			foreach ($files as $file) {
				...
				if (!in_array('extension/module/' . $extension, $hiden)) {
					...
					foreach ($modules as $module) {
						...
						'name'      => $module['name'],
						'status'    => (isset($setting_info['status']) && $setting_info['status']) ? '<span class="label label-success">' . $this->language->get('text_enabled') . '</span>' : '<span class="label label-danger">' . $this->language->get('text_disabled') . '</span>',
