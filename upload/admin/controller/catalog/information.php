...
class ControllerCatalogInformation extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			// сохраняем ID информационной страницы
			...
			// имя категории из языка админки/дефолтного
			$language_id = (int)$this->config->get('config_language_id');
			$name = '';
			if (!empty($this->request->post['information_description'][$language_id]['title'])) {
				$name = $this->request->post['information_description'][$language_id]['title'];
			} else {
				// запасной вариант: взять первое попавшееся имя
				$first = reset($this->request->post['information_description']);
				$name = $first['title'] ?? '';
			}
			...
			if (isset($this->request->post['apply']) && (int)$this->request->post['apply'] === 1) {
				...
				'user_token=' . $this->session->data['user_token'] . '&information_id=' . (int)$information_id . $url,
				true
				));
			}
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if (isset($this->request->post['apply']) && (int)$this->request->post['apply'] === 1) {
				...
				$this->response->redirect($this->url->link(
				'catalog/information/edit',
				'user_token=' . $this->session->data['user_token'] . '&information_id=' . $this->request->get['information_id'] . $url,
				true
				));
			}
