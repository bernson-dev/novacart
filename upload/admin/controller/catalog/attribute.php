...
class ControllerCatalogAttribute extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if (isset($this->request->post['apply']) && $this->request->post['apply'] == 1) {
				...
				$this->response->redirect($this->url->link('catalog/attribute/edit', 'user_token=' . $this->session->data['user_token'] . $url, true));
			} else {
	...
	protected function getList() {
		...
		if (isset($this->request->get['filter_attribute_group_id'])) {
			$filter_attribute_group_id = (int)$this->request->get['filter_attribute_group_id'];
		} else {
		...
		$data['sort_id'] = $this->url->link('catalog/attribute', 'user_token=' . $this->session->data['user_token'] . '&sort=ad.attribute_id' . $url, true);
	...
	protected function getForm() {
		...
		if (isset($this->request->post['attribute_group_id'])) {
			...
			$data['attribute_group_id'] = (int)$attribute_info['attribute_group_id'];
		} else {
