...
class ControllerCatalogManufacturer extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			// Сохранить и добавить еще (потоковое добавление)
			...
			if (isset($this->request->post['apply']) && (int)$this->request->post['apply'] === 1) {
				...
				'user_token=' . $this->session->data['user_token'] . '&manufacturer_id=' . (int)$manufacturer_id . $url,
				true
				));
			}

			//Сохранить: стандартное поведение переход в список
			$this->session->data['success'] = sprintf(
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if (isset($this->request->post['apply']) && (int)$this->request->post['apply'] === 1) {
				...
				$this->response->redirect($this->url->link(
				'catalog/manufacturer/edit',
				'user_token=' . $this->session->data['user_token'] . '&manufacturer_id=' . $this->request->get['manufacturer_id'] . $url,
				true
				));
			}
	...
	protected function getList() {
		...
		foreach ($results as $result) {
			$data['manufacturers'][] = array(
				...
				//'status'         => (bool)$result['status'], // в производителях нет статуса. Добавить?
	...
	protected function getForm() {
		...
		*/
		...
		// оптимизация кода нужно заменить везде: не дублируем код, а просто выбираем путь к картинке и вызываем  resize.
		// Сначала задаём placeholder
		$data['placeholder'] = $this->model_tool_image->resize('no_image.png', 100, 100);

		// Определяем, какое изображение использовать
		$image = null;

		if (!empty($this->request->post['image']) && is_file(DIR_IMAGE . $this->request->post['image'])) {
			$image = $this->request->post['image'];
		} elseif (!empty($manufacturer_info) && !empty($manufacturer_info['image']) && is_file(DIR_IMAGE . $manufacturer_info['image'])) {
			$image = $manufacturer_info['image'];
		}

		// thumb либо resize выбранного изображения, либо placeholder
		$data['thumb'] = $image ? $this->model_tool_image->resize($image, 100, 100) : $data['placeholder'];
	...
	protected function validateForm() {
		...
		if (isset($this->request->post['manufacturer_seo_url'])) {
			$this->request->post['manufacturer_seo_url'] = $this->model_design_seo_url->normalizeLanguageScopedSeoUrls(
				$this->request->post['manufacturer_seo_url']
			);
		}
