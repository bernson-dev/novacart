...
class ControllerCatalogProduct extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			// название берём из языка админки/дефолтного
			$language_id = (int)$this->config->get('config_language_id');
			$name = '';
			if (!empty($this->request->post['product_description'][$language_id]['name'])) {
				$name = $this->request->post['product_description'][$language_id]['name'];
			} else {
				// запасной вариант: взять первое попавшееся имя
				$first = reset($this->request->post['product_description']);
				$name = $first['name'] ?? '';
			}

			// Сохранить и добавить (потоковое добавление)
			...
			if (isset($this->request->post['apply']) && (int)$this->request->post['apply'] === 1) {
				...
				'user_token=' . $this->session->data['user_token'] . '&product_id=' . (int)$product_id . $url,
				true
				));
			}
	...
	public function edit() {
		$this->document->addScript('view/javascript/jquery/Sortable.min.js');
		$this->load->language('catalog/product');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$data['button_fill_empty_fields'] = $this->language->get('button_fill_empty_fields');
			...
			if (isset($this->request->post['apply']) && (int)$this->request->post['apply'] === 1) {
				...
				$this->response->redirect($this->url->link(
				'catalog/product/edit',
				'user_token=' . $this->session->data['user_token'] . '&product_id=' . (int)$this->request->get['product_id'] . $url,
				true
				));
			}
...
//		}
		...
		if (isset($this->request->get['sort'])) {
			...
			$sort = 'p.product_id'; // сортировка по ID
		}
		...
		// FilterImageId
		$data['sort_id'] = $this->url->link('catalog/product', 'user_token=' . $this->session->data['user_token'] . '&sort=p.product_id' . $url, true);
		// FilterImageId end
...
$data['view'] = isset($this->request->get['product_id'])
? sprintf('/index.php?route=product/product&product_id=%s', $this->request->get['product_id'])
: false;
...
*/
		...
		$data['view'] = $product_id ? "/index.php?route=product/product&product_id={$product_id}" : false;

		$data['action'] = empty($product_id) ? $this->url->link('catalog/product/add', 'user_token=' . $this->session->data['user_token'] . $url, true) : $this->url->link('catalog/product/edit', 'user_token=' . $this->session->data['user_token'] . '&product_id=' . (int)$product_id . $url, true);
		...
		// ===== Вывод поля комментарий (начало)
		if (isset($this->request->post['comment'])) {
			$data['comment'] = $this->request->post['comment'];
		} elseif (!empty($product_info) && isset($product_info['comment'])) {
			$data['comment'] = $product_info['comment'];
		} else {
			$data['comment'] = '';
		}
		// ===== Вывод поля комментарий (конец)
		...
		// ===== Вывод кода валюты по умолчанию (начало)
		$currency_code = $this->config->get('config_currency');

		if ($currency_code) {
			$data['default_currency_code'] = $currency_code;
		} else {
			$data['default_currency_code'] = '';
		}
		// ===== Вывод кода валюты по умолчанию (конец)
		...
		if (isset($this->request->post['date_added'])) {
			...
			$data['date_added'] = ($product_info['date_added'] != '0000-00-00') ? date('Y-m-d', strtotime($product_info['date_added'])) : '';
		} else {
		...
		// ===== Добавлено для работы с полем автодополнения главная категория (начало)
		// Получение информации о категории
		$category_info = $this->model_catalog_category->getCategory($data['main_category_id']);

		// Формирование полного пути категории
		if ($category_info) {
			$data['main_category'] = ($category_info['path']) ? $category_info['path'] . ' &gt; ' . $category_info['name'] : $category_info['name'];
		} else {
			$data['main_category'] = '';
		}
		// ===== Добавлено для работы с полем автодополнения главная категория (конец)
		...
		try {
			if (!isset($this->request->post['product_id'])) {
				$this->response->setOutput(json_encode([
					'success' => false,
					'error' => $this->language->get('error_product_id_missing')
				]));
				return;
			}

			$this->load->model('catalog/product');
			$result = $this->model_catalog_product->fillEmptyFields((int)$this->request->post['product_id']);

			if ($result === false) {
				$this->response->setOutput(json_encode([
					'success' => false,
					'error' => $this->language->get('error_product_not_found')
				]));
				return;
			}

			// Формируем ответ с локализованными сообщениями
			$response = [
				'success' => true,
				'copied' => $result['copied'],
				'source_language' => $result['source_language'],
				'copied_fields' => $result['copied_fields'],
				'filled_languages_stats' => $result['filled_languages_stats'] // Добавляем новую информацию
			];

			if ($result['copied']) {
				$response['message'] = sprintf(
				$this->language->get('text_fields_copied_from'),
				$result['source_language']['name'],
				$result['source_language']['code']
				);
			} elseif ($result['source_language']) {
				$response['message'] = sprintf(
				$this->language->get('text_all_fields_filled'),
				$result['source_language']['name'],
				$result['source_language']['code']
				);
			} else {
				$response['message'] = $this->language->get('text_no_fields_to_copy');
			}
			...
			$this->response->setOutput(json_encode([
				'success' => false,
				'error' => $this->language->get('error_server')
			]));
		}
	}
}
