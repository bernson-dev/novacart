...
class ControllerCatalogAttributeGroup extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if(isset($this->request->post['apply']) && $this->request->post['apply'] == 1) {
				$url .= '&attribute_group_id=' . $attribute_group_id;
				$this->response->redirect($this->url->link('catalog/attribute_group/edit', 'user_token=' . $this->session->data['user_token'] . $url, true));
			} else {
	...
	protected function getList() {
		...
		$data['sort_id'] = $this->url->link('catalog/attribute_group', 'user_token=' . $this->session->data['user_token'] . '&sort=agd.attribute_group_id' . $url, true);
