...
class ControllerCatalogRecurring extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$recurring_id = $this->model_catalog_recurring->addRecurring($this->request->post);
			...
			if(isset($this->request->post['apply']) && $this->request->post['apply'] == 1) {
				$url .= '&recurring_id=' . $recurring_id;
				$this->response->redirect($this->url->link('catalog/recurring/edit', 'user_token=' . $this->session->data['user_token'] . $url, true));
			} else {
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if(isset($this->request->post['apply']) && $this->request->post['apply'] == 1) {
				$url .= '&recurring_id=' . $this->request->get['recurring_id'];
				$this->response->redirect($this->url->link('catalog/recurring/edit', 'user_token=' . $this->session->data['user_token'] . $url, true));
			} else {
			$this->response->redirect($this->url->link('catalog/recurring', 'user_token=' . $this->session->data['user_token'] . $url, true));
			}
	...
	protected function getList() {
		...
		// FIX: Typo recurring controller https://github.com/opencart/opencart/commit/1f2d14565b18429a5d47410d88ec40baec869bae
