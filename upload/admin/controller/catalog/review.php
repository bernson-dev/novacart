...
class ControllerCatalogReview extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if (isset($this->request->post['apply']) && $this->request->post['apply'] == 1) {
				...
				$this->response->redirect($this->url->link('catalog/review/edit', 'user_token=' . $this->session->data['user_token'] . $url, true));
			} else {
	...
	protected function getForm() {
		...
		if (isset($this->request->post['images'])) {
			...
			$images = json_decode($review_info['images'], true);

			foreach ($images as $image) {
				$data['images'][] = array(
				'thumb' => $this->model_tool_image->resize($image['filename'], 100, 100),
				'popup' => $this->model_tool_image->resize($image['filename'], 1000, 1000),
	...
	public function removeImage() {
		...
		if (isset($this->request->post['review_id']) && isset($this->request->post['filename'])) {
			$this->load->model('catalog/review');

			$review_info = $this->model_catalog_review->getReview($this->request->post['review_id']);

			if ($review_info) {
				$images = json_decode($review_info['images'], true);

				foreach ($images as $key => $image) {
					if ($image['filename'] == $this->request->post['filename']) {
						unset($images[$key]);
					}
				}

				$this->model_catalog_review->editReviewImages($this->request->post['review_id'], $images);

				$json['success'] = $this->language->get('text_success');
			} else {
				$json['error'] = $this->language->get('error_review_image_delete') . ' no review_info';
			}
		} else {
			$json['error'] = $this->language->get('error_review_image_delete') . ' no params';
		}
