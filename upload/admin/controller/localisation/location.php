...
class ControllerLocalisationLocation extends Controller {
	...
	public function geocode() {
		...
		if (!empty($this->request->get['address'])) {
			...
			$this->load->model('localisation/location');
			$info = $this->model_localisation_location->getOSMAddressInfo($address);

			if ($info) {
				$json = array(
				'lat' => $info['lat'],
				'lng' => $info['lng'],
				'message' => $this->language->get('text_geocode_success')
				);
			} else {
				$json['error'] = $this->language->get('text_geocode_error');
			}
		} else {
			$json['error'] = $this->language->get('text_geocode_empty');
		}
