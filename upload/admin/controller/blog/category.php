...
class ControllerBlogCategory extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$blog_category_id = $this->model_blog_category->addCategory($this->request->post);
			...
			if (isset($this->request->post['apply']) && $this->request->post['apply'] == 1) {
				$url .= '&blog_category_id=' . $blog_category_id;
				$this->response->redirect($this->url->link('blog/category/edit', 'user_token=' . $this->session->data['user_token'] . $url, true));
			} else {
	...
	protected function validateForm() {
		...
		if (isset($this->request->post['category_seo_url'])) {
			$this->request->post['category_seo_url'] = $this->model_design_seo_url->normalizeLanguageScopedSeoUrls(
				$this->request->post['category_seo_url']
			);
		}
	...
	//Fix: метод нигде неиспользуется
