...
class ControllerCommonDashboard extends Controller {
	...
	public function removeInstall() {
		...
		$install_path = DIR_APPLICATION . '../install';

		if (is_dir($install_path) && basename($install_path) === 'install') {
			if ($this->deleteDirectory($install_path, $errors)) {
			...
			$json['error'] = $this->language->get('text_install_not_found');
		}
	...
	private function deleteDirectory($dir, &$errors = []) {
		if (!is_dir($dir)) {
			$errors[] = "Не папка: {$dir}";
			return false;
		}
		...
		foreach ($objects as $object) {
			...
			if (is_dir($filePath)) {
				$this->deleteDirectory($filePath, $errors);
			} elseif (is_file($filePath)) {
				if (!@unlink($filePath)) {
					$errors[] = "Ошибка удаления файла: {$filePath}";
				}
			}
		}

		if (!@rmdir($dir)) {
			$errors[] = "Ошибка удаления папки: {$dir}";
			return false;
		}
