...
class ControllerSaleOrder extends Controller {
	...
	public function delete() {
		...
		if (isset($this->request->get['filter_order_phone'])) {
			$url .= '&filter_order_phone=' . urlencode(html_entity_decode($this->request->get['filter_order_phone'], ENT_QUOTES, 'UTF-8'));
		}

		if (isset($this->request->get['filter_order_email'])) {
			$url .= '&filter_order_email=' . urlencode(html_entity_decode($this->request->get['filter_order_email'], ENT_QUOTES, 'UTF-8'));
		}
	...
	protected function getList() {
		...
		if (isset($this->request->get['filter_order_phone'])) {
			$filter_order_phone = $this->request->get['filter_order_phone'];
		} else {
			$filter_order_phone = '';
		}
		...
		'filter_order_phone'     => $filter_order_phone,
		'filter_order_email'     => $filter_order_email,
		...
		// TODO исправить запрос в модели для получения телефона и eMail (вынести из цикла)
			$order_phone_email = $this->model_sale_order->getOrderPhoneMail($result['order_id']);
			$data['orders'][] = array(
			'order_id'      => $result['order_id'],
			'store_name'    => $result['store_name'],
			'order_phone'   => $order_phone_email['phone'],
			'order_email'   => $order_phone_email['email'],
		...
		$data['sort_store_name'] = $this->url->link('sale/order', 'user_token=' . $this->session->data['user_token'] . '&sort=o.store_name' . $url, true);
		...
		$data['filter_order_phone'] = $filter_order_phone;
		$data['filter_order_email'] = $filter_order_email;
	...
	public function info() {
		...
		if ($order_info) {
			...
			$phone_clean = preg_replace('/[^0-9]/', '', $order_info['telephone']);
			$phone_for_search = isset($order_info['telephone']) ? $order_info['telephone'] : '';
			$email_for_search = isset($order_info['email']) ? $order_info['email'] : '';
			$orders_count = $this->model_sale_order->getOrdersCountByPhoneEmail($phone_for_search, $email_for_search);
			$phone = isset($order_info['telephone']) ? trim($order_info['telephone']) : '';
			$email = isset($order_info['email']) ? trim($order_info['email']) : '';
			$data['phone_orders_count'] = $this->model_sale_order->getOrdersCountByPhone($phone);
			$data['email_orders_count'] = $this->model_sale_order->getOrdersCountByEmail($email);
			$data['has_other_orders'] = ($data['phone_orders_count'] > 1 || $data['email_orders_count'] > 1);
			$data['order_phone_url'] = $this->url->link('sale/order', 'user_token=' . $this->session->data['user_token'] . '&filter_order_phone=' . $phone_clean, true);
			$data['order_email_url'] = $this->url->link('sale/order', 'user_token=' . $this->session->data['user_token'] . '&filter_order_email=' . $order_info['email'], true);
	...
	public function history() {
		...
		$results = $this->model_sale_order->getOrderHistories($order_id, ($page - 1) * $limit, $limit);
	...
	public function customer_autocomplete() {
		...
		if (isset($this->request->get['filter_name'])) {
			...
			$results = $this->model_sale_order->getOrderCustomers($filter_name);

			foreach ($results as $result) {
				$json[] = array(
				'label' => $result['customer'],
	...
	public function telephone_autocomplete() {
		$json = array();

		if (isset($this->request->get['filter_name'])) {
			$filter_name = preg_replace('/[^0-9]/', '', $this->request->get['filter_name']);

			$this->load->model('sale/order');

			$results = $this->model_sale_order->getOrderTelephones($filter_name);

			foreach ($results as $result) {
				$json[] = array(
				'label' => $result['raw'],       // красиво показываем
				'value' => $result['telephone']  // выбираем цифры
				);
			}
		}
