...
class ControllerUserUserPermission extends Controller {
	...
	protected function validateForm() {
		...
		// ===== Foolproof: защита группы Administrator (user_group_id=1) =====
		$user_group_id = (int)($this->request->get['user_group_id'] ?? 0);

		if ($user_group_id === 1) {

			// Критические роуты, которые нельзя снять
			$critical_routes = [
			...
			$modify = $this->request->post['permission']['modify'] ?? [];

			$restored = [];     // что именно мы вернули
			$has_error = false; // была ли ошибка

			// Запрет "снять всё"
			if (empty($access) || empty($modify)) {
				$this->error['warning'] = $this->language->get('error_admin_empty_permissions');
				$has_error = true;
			}

			// Проверяем и восстанавливаем критические access
			foreach ($critical_routes as $route) {
				if (!in_array($route, $access, true)) {
					$access[] = $route;
					$restored[] = $route . ' (access)';
					$has_error = true;
				}
			}

			// Проверяем и восстанавливаем критические modify
			foreach ($critical_routes as $route) {
				if (!in_array($route, $modify, true)) {
					$modify[] = $route;
					$restored[] = $route . ' (modify)';
					$has_error = true;
				}
			}

			// Если что-то восстанавливали — записываем обратно в POST, чтобы форма отрисовалась с флажками
			if ($restored) {
				$this->request->post['permission']['access'] = $access;
				$this->request->post['permission']['modify'] = $modify;
			}

			// Сообщение пользователю: почему не сохранилось и что восстановили
			if ($has_error) {
				// Если не было конкретного warning — ставим общий
				if (!isset($this->error['warning'])) {
					$this->error['warning'] = $this->language->get('text_admin_permissions_restored');
				} else {
					// Добавим вторую строку (в пределах админского alert)
					$this->error['warning'] .= ' ' . $this->language->get('text_admin_permissions_restored');
				}
			}
		}
