...
class ControllerUserApi extends Controller {
	...
	protected function getForm() {
		...
		$data['is_new'] = !isset($this->request->get['api_id']);
	...
	protected function validateForm() {
		...
		if (isset($this->request->post['api_ip']) && is_array($this->request->post['api_ip'])) {
			foreach ($this->request->post['api_ip'] as $ip) {
				if (trim($ip) !== '') {
					$has_ip = true;
					break;
				}
		...
		if (!isset($this->error['warning']) && !$has_ip) {
	...
	public function deleteSession() {
		...
		} elseif (!isset($this->request->get['api_session_id'])) {
			$json['error'] = $this->language->get('error_warning');
		} else {
			$this->load->model('user/api');

			$this->model_user_api->deleteApiSession((int)$this->request->get['api_session_id']);
