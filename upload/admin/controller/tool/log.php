...
class ControllerToolLog extends Controller {
	...
	public function index() {
		...
		if (is_file($file)) {
			...
			// Список суффиксов для размера
			$suffix = array('B','KB','MB','GB','TB','PB','EB','ZB','YB');
			$i = 0;
			$display_size = $size;

			while (($display_size / 1024) > 1 && $i < count($suffix)-1) {
				$display_size /= 1024;
				$i++;
			}

			if ($size >= 5242880) {
				// Файл больше 5 MB → предупреждение и чтение последних 3 MB
				$data['error_warning'] = sprintf(
				$this->language->get('error_warning'),
				basename($file),
				round($display_size, 2) . $suffix[$i]
				);

				$handle = fopen($file, 'r');
				// Смещаемся к концу файла на 3 MB
				fseek($handle, -3145728, SEEK_END);
				$data['log'] = htmlspecialchars(fread($handle, 3145728), ENT_COMPAT, 'UTF-8');
				fclose($handle);
			} else {
