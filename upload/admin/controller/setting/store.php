...
class ControllerSettingStore extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->load->model('setting/setting');
			$this->model_setting_setting->editSetting('config', $this->request->post, (int)$store_id);

			// Генерация favicon ПОСЛЕ сохранения настроек
			$this->load->model('tool/favicon_generator');

			$svg      = isset($this->request->post['config_svg_icon']) ? $this->request->post['config_svg_icon'] : '';
			$color    = isset($this->request->post['config_favicon_color']) ? $this->request->post['config_favicon_color'] : '#000000';
			...
			if (isset($this->request->post['apply']) && $this->request->post['apply'] == 1) {
				...
				$this->response->redirect($this->url->link('setting/store/edit', 'user_token=' . $this->session->data['user_token'] . $url, true));
			} else {
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->load->model('setting/setting');
			$this->model_setting_setting->editSetting('config', $this->request->post, (int)$this->request->get['store_id']);
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			...
			foreach ($this->request->post['selected'] as $store_id) {
				...
				// Удаляем папку favicon набора магазина
				$this->model_tool_favicon_generator->clearStore($store_id);
			}
	...
	protected function getForm() {
		// чтобы не было Notice, когда store_id не задан
		$store_info = array();
		...
		// SVG favicon preview endpoint
		$data['svg_preview_url'] = $this->url->link(
		'tool/svg_preview',
		...
		if (isset($this->request->post['config_favicon_original'])) {
			...
			$data['config_favicon_original'] = isset($store_info['config_favicon_original']) ? $store_info['config_favicon_original'] : 0;
		}

		if (isset($this->request->post['config_svg_icon'])) {
			$data['config_svg_icon'] = $this->request->post['config_svg_icon'];
		} else {
			$data['config_svg_icon'] = isset($store_info['config_svg_icon']) ? $store_info['config_svg_icon'] : '';
		}

		if (isset($this->request->post['config_favicon_color'])) {
			$data['config_favicon_color'] = $this->request->post['config_favicon_color'];
		} else {
			$data['config_favicon_color'] = !empty($store_info['config_favicon_color']) ? $store_info['config_favicon_color'] : '#ff0000';
		}
		...
		if (!empty($data['config_svg_icon']) && is_file(DIR_IMAGE . $data['config_svg_icon'])) {
			...
			if ($ext === 'svg') {
				$data['svg_icon'] = $data['svg_preview_url'] . '&path=' . urlencode($data['config_svg_icon']);
		...
		$data['error_svg_icon'] = isset($this->error['svg_icon']) ? $this->error['svg_icon'] : '';
		$data['error_favicon_color'] = isset($this->error['favicon_color']) ? $this->error['favicon_color'] : '';
