...
class ControllerCustomerCustomField extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if (isset($this->request->post['apply']) && $this->request->post['apply'] == 1) {
				$url .= '&custom_field_id=' . $custom_field_id;
				$this->response->redirect($this->url->link('customer/custom_field/edit', 'user_token=' . $this->session->data['user_token'] . $url, true));
			} else {
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if (isset($this->request->post['apply']) && $this->request->post['apply'] == 1) {
				$url .= '&custom_field_id=' . $this->request->get['custom_field_id'];
				$this->response->redirect($this->url->link('customer/custom_field/edit', 'user_token=' . $this->session->data['user_token'] . $url, true));
			} else {
				$this->response->redirect($this->url->link('customer/custom_field', 'user_token=' . $this->session->data['user_token'] . $url, true));
			}
