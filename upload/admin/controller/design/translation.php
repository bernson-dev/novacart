...
class ControllerDesignTranslation extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$translation_id = $this->model_design_translation->addTranslation($this->request->post);
			...
			if(isset($this->request->post['apply']) && $this->request->post['apply'] == 1) {
				$url .= '&translation_id=' . $translation_id;
				$this->response->redirect($this->url->link('design/translation/edit', 'user_token=' . $this->session->data['user_token'] . $url, true));
			} else {
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if(isset($this->request->post['apply']) && $this->request->post['apply'] == 1) {
				$url .= '&translation_id=' . $this->request->get['translation_id'];
				$this->response->redirect($this->url->link('design/translation/edit', 'user_token=' . $this->session->data['user_token'] . $url, true));
			} else {
			$this->response->redirect($this->url->link('design/translation', 'user_token=' . $this->session->data['user_token'] . $url, true));
			}
