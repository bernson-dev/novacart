...
$_['text_information']     = 'Информация<p><strong>SEO URL (keyword)</strong> — это уникальный идентификатор для URL-адресов. Он должен быть уникален для каждого объекта (товара, категории, производителя) в пределах одного магазина и языка. Используйте только буквы латинского алфавита в нижнем регистре, цифры и дефис (-). Например: <code>my-product-1</code>.</p><p>С точки зрения SEO - лучше использовать уникальные <strong>keyword</strong> для каждого языка, чтобы улучшить ранжирование и видимость сайта.</p><p>Не рекомендуется использовать одинаковые <strong>keyword</strong> для разных языков, так как может создаваться дублированный контент для поисковых систем, особенно если они индексируют обе языковые версии.</p>';
...
$_['text_audit_summary']       = 'всего записей: %d; проблемных записей: %d; групп конфликтующих SEO URL: %d; групп дублирующихся ссылок: %d; конфликтов с языковыми префиксами: %d.';
