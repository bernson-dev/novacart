<?php
// *	@source		See SOURCE.txt for source and other copyright.
// *	@license	GNU General Public License version 3; see LICENSE.txt

// Heading
$_['heading_title']     = 'Файлы для скачивания';

// Text
$_['text_success']      = 'Настройки успешно изменены!';
$_['text_list']         = 'Список файлов для скачивания';
$_['text_add']          = 'Добавить';
$_['text_edit']         = 'Редактирование';
$_['text_upload']       = 'Файл успешно загружен!';
$_['text_report']       = 'Отчёт';

// Column
$_['column_name']       = 'Название';
$_['column_ip']         = 'IP';
$_['column_account']    = 'Аккаунты';
$_['column_store']      = 'Магазин';
$_['column_date_added'] = 'Дата добавления';
$_['column_action']     = 'Действие';

// Entry
$_['entry_name']        = 'Название';
$_['entry_filename']    = 'Имя файла';
$_['entry_mask']        = 'Маска';

// Help
$_['help_filename']     = 'Вы можете закачать файл через кнопку выбора файла или по FTP, поместив его в папку "download" и введя информацию ниже.';
$_['help_mask']         = 'Рекомендуется, чтобы имя файла и маска отличались, это не позволит получить прямой доступ к файлу.';

// Error
$_['error_warning']     = 'Предупреждение: Внимательно проверьте форму на наличие ошибок!';
$_['error_permission']  = 'У вас недостаточно прав для внесения изменений!';
$_['error_name']        = 'Название должно содержать от 3 до 64 символов!';
$_['error_upload']      = 'Необходимо загрузить файл!';
$_['error_filename']    = 'Системное имя файла должно содержать от 3 до 160 символов!';
$_['error_upload_filename'] = 'Имя загружаемого файла должно содержать от 3 до 127 символов!';
$_['error_exists']      = 'Файл не найден!';
$_['error_mask']        = 'Маска должна содержать от 3 до 128 символов!';
$_['error_filetype']    = 'Неправильный тип файла!';
$_['error_product']     = 'Эту загрузку нельзя удалить, так как она используется в %s товарах!';
$_['error_article']     = 'Эту загрузку нельзя удалить, так как она используется в %s статьях!';
$_['error_upload_size'] = 'Размер файла не должен превышать %s МБ!';
$_['error_upload_post_max'] = 'Размер загружаемых данных превышает лимит PHP post_max_size (%s МБ)!';
$_['error_upload_1'] = 'Файл превышает максимально допустимый сервером размер (upload_max_filesize)!';
$_['error_upload_2'] = 'Файл превышает максимально допустимый размер, указанный в форме!';
$_['error_upload_3'] = 'Файл был загружен только частично!';
$_['error_upload_4'] = 'Файл не был загружен!';
$_['error_upload_6'] = 'Временная папка для загрузки файлов отсутствует!';
$_['error_upload_7'] = 'Не удалось записать файл на диск!';
$_['error_upload_8'] = 'Загрузка файла остановлена расширением PHP!';
