...
class ModelBlogReview extends Model {
	public function addReview($article_id, $data) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "review_article SET author = '" . $this->db->escape($data['name']) . "', customer_id = '" . (int)$this->customer->getId() . "', article_id = '" . (int)$article_id . "', text = '" . $this->db->escape($data['text']) . "', rating = '" . (int)$data['rating'] . "', status = '0', date_added = NOW()");
		...
		if ($this->config->get('configblog_review_mail')) {
			...
			if ($article_info) {
				...
				$message .= sprintf($this->language->get('text_rating'), (int)$data['rating']) . "\n";
