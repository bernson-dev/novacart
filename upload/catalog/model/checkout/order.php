...
class ModelCheckoutOrder extends Model {
	public function addOrder($data) {
		...
		if (isset($data['products'])) {
			// --- cost_price map for all products in this order (1 query) ---
			$cost_price_map = [];
			...
			if ($product_ids) {
				...
				$q = $this->db->query("SELECT product_id, cost_price FROM " . DB_PREFIX . "product WHERE product_id IN (" . $ids_sql . ")");
				foreach ($q->rows as $row) {
					$cost_price_map[(int)$row['product_id']] = (float)$row['cost_price'];
				}
			}
			// --- /cost_price map ---

			foreach ($data['products'] as $product) {
				$cost_price = isset($cost_price_map[(int)$product['product_id']])
				? (float)$cost_price_map[(int)$product['product_id']]
				: 0.0000;

				$this->db->query("INSERT INTO " . DB_PREFIX . "order_product SET order_id = '" . (int)$order_id . "', product_id = '" . (int)$product['product_id'] . "', name = '" . $this->db->escape($product['name']) . "', model = '" . $this->db->escape($product['model']) . "', quantity = '" . (int)$product['quantity'] . "', price = '" . (float)$product['price'] . "',     cost_price = '" . (float)$cost_price . "', total = '" . (float)$product['total'] . "', tax = '" . (float)$product['tax'] . "', reward = '" . (int)$product['reward'] . "'");
