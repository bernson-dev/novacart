...
class ControllerExtensionPaymentTwoCheckoutCplus extends Controller {
	...
	public function checkout() {
		...
		if (!empty($post_data['order_id'])) {
			...
			$other_details = $this->model_extension_payment_twocheckout_cplus->getOtherDetails($order_info, $test, $seller_id, $language);
			$hasRecurring = $this->cart->hasRecurringProducts();
			$products = $this->model_extension_payment_twocheckout_cplus->getProducts(
				$order_info['order_id'],
			...
			$buy_link_params = array_merge($buy_link_params, $products);
	...
	public function success() {
		...
		$api_response = $this->model_extension_payment_twocheckout_cplus->call('orders/' . $params['refno']);

		if ((!empty($api_response['RefNo']) && isset($api_response['RefNo'])) &&
			(!empty($api_response['Status']) && isset($api_response['Status']))) {
			if (in_array($api_response['Status'], ['AUTHRECEIVED', 'COMPLETE'])) {
				$products = $api_response['Items'];
				foreach ($products as $item) {
					if (count($item['ProductDetails']['Subscriptions']) &&
						$item['ProductDetails']['Subscriptions'][0]['RecurringEnabled']) {
						$product_id = $item['ExternalReference'];
						$idsArray = explode('_', $product_id);
						$recurringOrderId = end($idsArray);
						$reference = $item['ProductDetails']['Subscriptions'][0]['SubscriptionReference'];
						$this->model_extension_payment_twocheckout_cplus->updateRecurring($recurringOrderId, $reference);
						$this->model_extension_payment_twocheckout_cplus->addRecurringTransaction($recurringOrderId, $reference, $order_info['total'], 1);
					}
				}
