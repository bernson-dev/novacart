...
class ControllerStartupSession extends Controller {
	public function index() {
		if (isset($this->request->get['api_token']) && isset($this->request->get['route']) && substr($this->request->get['route'], 0, 4) == 'api/') {
			...
			// Проверка IP и токена
			$api_query = $this->db->query("SELECT DISTINCT * FROM `" . DB_PREFIX . "api` `a`
                LEFT JOIN `" . DB_PREFIX . "api_session` `as` ON (a.api_id = as.api_id)
                LEFT JOIN `" . DB_PREFIX . "api_ip` `ai` ON (a.api_id = ai.api_id)
                WHERE a.status = '1'
                AND `as`.`session_id` = '" . $this->db->escape((string)$this->request->get['api_token']) . "'
                AND ai.ip = '" . $this->db->escape((string)$this->request->server['REMOTE_ADDR']) . "'");
