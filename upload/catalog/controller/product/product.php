...
class ControllerProductProduct extends Controller {
	...
	public function index() {
		...
		if ($product_info) {
			...
			if (is_file(DIR_IMAGE . html_entity_decode($product_info['image'], ENT_QUOTES, 'UTF-8'))) {
				...
				// Исправленная проверка метода
				if (method_exists($this->document, 'setOgImage')) {
	...
	public function write() {
		...
		if (!$this->config->get('config_review_status')) {
		...
		} elseif (!$this->config->get('config_review_guest') && !$this->customer->isLogged()) {
			...
			if (!isset($json['error'])) {
				...
				$this->request->post['rating'] = $rating;
